...
void AppStage_DistortionCalibration::renderUI()
{
	...
	case eMenuState::showWarning:
	{
		...
		ImGui::Separator();
		if (AssetManager::ImGuiButtonIcon(AssetManager::getInstance()->getIconCheck(), "Continue"))
		{
    ...
    case eMenuState::capture:
        {
...

            {
				...
				else
				{
					...
					ImGui::SameLine();
					ImGui::TextColored(AssetManager::k_imcolor_red(), "Capture point not valid!");
				}

				ImGui::Separator();
                if (AssetManager::ImGuiButtonIcon(AssetManager::getInstance()->getIconUpdate(), "Restart"))
                {
