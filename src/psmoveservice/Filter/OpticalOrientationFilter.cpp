...
Eigen::Quaternionf OpticalOrientationFilter::getOrientation(float time, float offset_x, float offset_y, float offset_z, float offset_world_x, float offset_world_y, float offset_world_z) const
{
	...
	if (m_state->bIsValid)
	{
		Eigen::Quaternionf orientation = getCombinedOrientation();
...
Eigen::Vector3f OpticalOrientationFilter::getAngularVelocityRadPerSec(float offset_x, float offset_y, float offset_z) const
{
	if (!m_state->bIsValid)
...
Eigen::Vector3f OpticalOrientationFilter::getAngularAccelerationRadPerSecSqr(float offset_x, float offset_y, float offset_z) const
{
	if (!m_state->bIsValid)
		return Eigen::Vector3f::Zero();
	...
	if (packet.has_imu_measurements())
	{
		if (packet.controllerDeviceId > -1)
		{
			...
			if (ControllerView != nullptr && OtherControllerView != nullptr && ControllerView->getIsOpen() && OtherControllerView->getIsOpen())
			{
				...
				m_state->apply_optical_state(forwardQuat_inv, timestamp);
			}
		}
	}
#endif
